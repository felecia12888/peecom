...
from .sensor_validation import AdvancedSensorValidator, PS4Corrector, monitor_sensor_health


class SensorMonitor:
    def __init__(self):
        self.validator = AdvancedSensorValidator()
    ...
    def validate_ps4_readings(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if anomalies.any():
            self.log_alert(
                f"PS4 anomalies detected at indices: {np.where(anomalies)[0]}")

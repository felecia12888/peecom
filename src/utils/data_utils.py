...
# Default directories - will be overridden by function parameters
DEFAULT_FIGURES_DIR = "output/figures"
...
def analyze_sensor_statistics(data, sensor_config, figures_dir=None):
    ...
    if sensor_config.get('unit') == 'bar':  # Only for pressure sensors
        ...
        plt.hist(values.flatten(), bins=50, range=(
            0, np.percentile(values, 99)))
        plt.title(f'Distribution of Readings (including zeros)')
        plt.xlabel('Pressure (bar)')
        plt.ylabel('Frequency')
        plt.savefig(os.path.join(
            figures_dir, f'zero_analysis_{data.name if hasattr(data, "name") else "pressure"}.png'))
        plt.close()
    return stats
        ...
        if 'statistics' in data and 'zero_analysis' in data['statistics']:
            ...
            if sensor == 'PS4' and status == 'Critical':
                recommendations.append(
                    "IMMEDIATE ACTION: Calibrate and verify sensor functionality")
                recommendations.append(
                    "Check for physical blockages or electrical issues")
                ...
                recommendations.append(
                    "Regular flow rate validation recommended")
            elif sensor.startswith('TS'):
                recommendations.append(
                    "Verify temperature correlation patterns")
...
def analyze_temporal_patterns(data, sensor_type, figures_dir=None):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(
        figures_dir, f'{sensor_type}_temporal_patterns.png'))
    plt.close()
        ...
        for sensor_name, data in sensor_data.items():
            try:
                if isinstance(data, pd.DataFrame) and not data.empty:
                    ...
                    window_size = min(100, values.shape[0])
                    rolled_mean = pd.DataFrame(values).rolling(
...
def check_sensor_correlations(all_sensor_data, figures_dir=None):
    ...
    try:
        ...
        with tqdm(total=total_sensors, desc="Computing time series correlations", ncols=100) as pbar:
            for name, data in all_sensor_data.items():
                if isinstance(data, pd.DataFrame) and not data.empty:
                    ...
                    rolled = pd.DataFrame(values).rolling(
                        window=window_size, min_periods=1).mean()
                    ...
                    if np.std(rolled) > 1e-10:
                        ...
                        sensor_means[name] = np.zeros_like(
                            rolled.mean(axis=1).values)
                else:
        ...
        with tqdm(total=2, desc="Calculating temporal correlations", ncols=100) as pbar:
            ...
            means_array = np.nan_to_num(
                means_array, nan=0.0, posinf=0.0, neginf=0.0)
            pbar.update(1)
            ...
            try:
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(
                    figures_dir, 'sensor_correlations.png'))
                plt.close()
                pbar.update(1)
...
def analyze_condition_distribution(profile, figures_dir=None):
...
def save_analysis_results(results, dataset_name, txt_results_file=None, csv_results_file=None):
    """Save analysis results to files"""
    if txt_results_file is None:
        txt_results_file = os.path.join(
            DEFAULT_ANALYSIS_DIR, "dataset_analysis_results.txt")
    if csv_results_file is None:
        csv_results_file = os.path.join(
            DEFAULT_ANALYSIS_DIR, "dataset_analysis_results.csv")

    # Ensure directories exist
    os.makedirs(os.path.dirname(txt_results_file), exist_ok=True)
    os.makedirs(os.path.dirname(csv_results_file), exist_ok=True)
...
class SensorValidator:
    ...
    def validate_sensor(self, sensor_id: str, data: np.ndarray, related_sensors: Dict = None) -> Dict:
        ...
        if sensor_type == 'PS':
            physical_check = self.validate_physical_readings(
                data, sensor_type, (0, 200))  # bar
            if physical_check['out_of_range']:
                validation_result['alerts'].append(
                    f"Physical range violations detected: {physical_check['violation_count']} instances")
        ...
        if electrical_check['possible_connection_issue']:
            validation_result['alerts'].append(
                "Potential electrical connection issues detected")
            validation_result['status'] = 'Warning'
        ...
        if related_sensors and sensor_id == 'PS4':
            correlation_check = self.check_sensor_correlation(
                data, related_sensors)
            if not correlation_check['correlation_valid']:
                validation_result['alerts'].append(
                    "Correlation with related sensors below threshold")
                validation_result['status'] = 'Warning'
            ...
            if validation['corrections_needed']:
                if sensor_id == 'PS4':
                    if 'PS3' in data.columns and 'PS5' in data.columns:
                        ...
                        mask = (data[sensor_id] == 0) & (
                            confidence >= self.confidence_threshold)
                        corrected_data.loc[mask,
                                           sensor_id] = estimated_values[mask]
...
def analyze_sensor_health(df: pd.DataFrame) -> Dict:
    ...
    for column in df.columns:
        if column.startswith(('PS', 'TS', 'FS', 'VS', 'CE', 'CP', 'SE')):
            health_report[column] = validator.validate_sensor(
        ...
        if health['status'] == 'Critical':
            ...
            if sensor_type == 'PS':  # Pressure sensors
                action['recommendations'].append(
                    "Check hydraulic pressure system")
...
class PS4Monitor:
    ...
    def generate_report(self, original_data, corrected_data, related_sensors, figures_dir=None):
...
def analyze_dataset(data_dir: str, output_dir: str = None, figures_dir: str = None, analysis_suffix: str = "default") -> Dict:
    ...
    # Define result file paths with suffix
    txt_results_file = os.path.join(
        output_dir, f"dataset_analysis_results_{analysis_suffix}.txt")
    csv_results_file = os.path.join(
        output_dir, f"dataset_analysis_results_{analysis_suffix}.csv")
            ...
            for i in range(1, config['count'] + 1):
                try:
                    ...
                    if not data.empty:
                        ...
                        results[sensor_name] = {
                            'statistics': analyze_sensor_statistics(data, config, figures_dir),
                            'sampling': check_sampling_consistency(data, config)
                        }
                        analyze_temporal_patterns(
                            data, sensor_name, figures_dir)
                except Exception as e:
                    print(
                        f"Warning: Error analyzing {sensor_type}{i}: {str(e)}")
                finally:
                    pbar.update(1)
    ...
    try:
        with tqdm(total=1, desc="Loading profile data", ncols=100) as pbar:
            ...
            results['condition_analysis'] = analyze_condition_distribution(
                profile, figures_dir)
            pbar.update(1)
    except Exception as e:
    ...
    with tqdm(total=4, desc="Performing analysis steps", ncols=100) as pbar:
        results['sensor_correlations'] = check_sensor_correlations(
            all_sensor_data, figures_dir)
        pbar.update(1)
    ...
    with tqdm(total=1, desc="Saving results", ncols=100) as pbar:
        save_analysis_results(results, os.path.basename(
            data_dir), txt_results_file, csv_results_file)
        pbar.update(1)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="PEECOM Dataset Analysis")
    parser.add_argument('--dataset', default='dataset/dataset',
                        help='Dataset directory path')
    parser.add_argument('--output', default='output',
                        help='Output directory path')
    parser.add_argument('--analysis-name', default='default',
                        help='Analysis name suffix for output files')

    args = parser.parse_args()

    # Setup proper logging to output directory
    analysis_dir = os.path.join(args.output, "analysis")
    os.makedirs(analysis_dir, exist_ok=True)
    log_file = os.path.join(
        analysis_dir, f'dataset_check_{args.analysis_name}.log')
    ...
    try:
        results = analyze_dataset(args.dataset,
                                  ...
                                  figures_dir=os.path.join(
                                      args.output, "figures"),
                                  analysis_suffix=args.analysis_name)
        print(
            f"\nAnalysis completed successfully. Check the {analysis_dir}/ directory for results.")
        print(f"Logs saved to: {log_file}")
    except Exception as e:
        logging.error(f"Analysis failed: {str(e)}")
        print(f"Analysis failed. Check {log_file} for details.")
